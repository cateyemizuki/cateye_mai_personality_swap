...
写好人格 + 配好管理员 + 一条指令即可，全程持久化、重启不丢。

### 第 1 步：写好人格（预设）

一个人格 = 一个预设文件（表达方式 / 人格约束 / 行为风格）。两种做法：

- **让 AI 写**：生成 `preset/<名字>.toml` 放进**插件根目录的 `preset/`**（交付暂存区），
  插件下次加载时自动移入数据目录，**不用手动进数据目录放文件**；格式见下方
  「附：预设文件格式」。
- **从现有官方人格派生**：在聊天里发 `/maisave 主人格 0`（需管理员，见第 2 步），
  把当前官方人格存成预设，再照它改成想要的样子。

验证：发 `/mps list` 能看到预设名（公开命令，人人可用）。

### 第 2 步：配好管理员（决定"谁能下指令"）

管理命令（含下面的 `/mps swap`）只认三种身份：**本地 operator / 控制台**、
**管理员群里的任何成员**、**管理员 QQ 本人**。所以从 QQ 里下指令前，先打开
**WebUI → 插件配置页 → 「黑白名单与管理员」**，二选一（也可都填）：

| 配置项 | 填什么 | 效果 |
|---|---|---|
| 管理员群列表 | 群号，如 `123456789` | 该群内**任何人**都能下管理命令（个人 bot 最省事） |
| 管理员 QQ 列表 | 你的 QQ 号，如 `10001`（也接受 `qq:10001`） | 只有这个 QQ 能下管理命令 |

> 两个名单都留空 = 只有本地 operator / 控制台能用；从聊天里发管理命令会被拒绝
> 并提示（只读命令 `/mps status`、`/mps list` 不受限）。

### 第 3 步：用指令把人格路由到指定聊天流

**在目标聊天流里**（就是要换人格的那个群 / 私聊）发送：

```
/mps swap 普瑞赛斯        # 该聊天流切到这个预设（时长省略 = 用预设记录的默认时长）
/mps swap 普瑞赛斯 120    # 只持续 120 分钟，到点自动恢复主人格
/mps swap                 # 不带名称（或 0）= 切回主人格
/mps status               # 看替换状态、剩余时长、生效权重（公开命令；非管理员只列当前聊天流）
```

- 「仅触发聊天流替换」默认开启 → 只影响**你发指令的那个聊天流**，别的群不受影响；
  关掉它则全局切换（所有流共用一个状态）。
- 切换**持久化**：重启后仍然记得哪个群用的是哪个人格。
- 想撤销：在同一个聊天流再发一次 `/mps swap` 即可。

**这就是完整的基本用法**：不配触发条件 / 概率 / 权重 / 脚本时，插件不会自动换人格，
只在你下指令时换。自动替换、权重抽取、maips 脚本都是进阶玩法，需要时再看下文。

> 自动替换默认概率 0.1 且需关键词命中，不会干扰手动路由；想彻底关闭它，把配置页
> 「触发概率」设为 `0`。

### 附：预设文件格式（`preset/<名字>.toml`）

```toml
name = "普瑞赛斯"              # 仅展示用；权威名是文件名
persona_name = "普瑞赛斯"       # 可选：该人格下 bot 的自称（覆盖官方昵称）
expression = """说话方式……"""   # 注入表达（怎么说）
persona = """我是谁、红线……"""  # 人格约束
behavior = """何时说话……"""     # 行为风格（注入 planner）
duration_minutes = 0            # 0 = 一直用；>0 = 切到它后默认持续 N 分钟
```

- `persona_name` 留空 = 沿用官方 `bot.nickname`；配了它时 planner 外壳里的官方昵称
  与"当前身份提醒"也会一并用它（避免模型被历史发言带偏）。
- `duration_minutes` 只是「将来切到它时的**默认时长**」，**预设文件本身永不过期**。
- 文件名即预设名：中文 / 字母 / 数字 / 下划线 / 连字符，1~64 位。

## 命令

权限列：**公开** = 任何人可发；**管理员** = 本地 operator/控制台，或配置页
「黑白名单与管理员」中的管理员 QQ / 管理员群。

| 命令 | 权限 | 说明 |
|---|---|---|
| `/mps swap [名称] [时长]` | 管理员 | 切换当前聊天流人格（"仅触发聊天流替换"关闭时为全局切换）；**不带名称（留空或 `0`）即切回主人格**；`[时长]`（分钟）为本次切换的持续上限，**省略则回退用该预设保存的时长**（`0`=永久一直用）；预设文件本身不受影响、仍可反复切换 |
| `/mps status` | 公开 | 查看替换状态、剩余时长、生效预设权重（**非管理员只显示当前聊天流的明细**，外加"另有 N 个聊天流非主人格"的计数——状态键即群号/QQ 号，避免被用来枚举他人会话标识） |
| `/mps list`（等价 `/mps maisave list`、`/maisave list`） | 公开 | 列出全部预设名称 |
...
- **数据目录的 `preset/`（`<MaiBot根>\data\plugins\<id>\preset\`）** = **运行数据**：
  `/maisave` 保存与自动导入都落这里，`/mps list`、路由/切换脚本读的都是
...
- 主人格权重（默认 0.8）与**预设权重表**（不要求总和为 1）。预设权重表在配置页里是
  一个 **JSON 文本框**（`{"乐子人": 1.5, "普瑞赛斯": 2}`，也可一行一条
  `乐子人=1.5`）——之所以不是对象控件，是因为 WebUI 的对象控件会把默认值渲染成
  `[object Object]`；v1.4.2 及以前遗留的 `[weights.presets]` TOML 表仍兼容
  （自动转成 JSON 文本，值不丢）。生效值可用 `/mps status` 直接核对，也可用
  `/mps weight <名称> <权重>` 逐条设置（命令值优先）
- 注入行为：
...
在聊天里对麦麦发送（管理命令，需管理员权限——见上文「最快上手」第 2 步）：
...
> 删除/移动后 `/mps list` 不再列出它，但 bot 官方人格表现、其它预设、
...
1. 在服务器上发 `/mps maiload 主人格`（需管理员）——bot 会用合并转发把主人格内容发给你
...
4. 放好重启/重载插件后验证（下列命令均需管理员，见「最快上手」第 2 步）：
   `/mps script list` 无 ❌ → `/mps list` 能看到自动导入的预设 → 在目标聊天流
   发 `/mps swap <预设名>` 实测 → 插件日志出现「覆盖 system」行即生效。
